...
public sealed class PanoptoApiClient : IDisposable
{
        ...
        return await Task.Run(() =>
        {
            try
            {
                ...
                using (new OperationContextScope(_authClient.InnerChannel))
                {
                    ...
                    if (properties[HttpResponseMessageProperty.Name] is HttpResponseMessageProperty httpResponse)
                    {
                        var setCookieHeader = httpResponse.Headers[HttpResponseHeader.SetCookie];
    ...
    public async Task<string> GetRemoteRecorderStateAsync(Guid remoteRecorderId)
    {
        ...
        if (_soapAuthInfo != null)
            return await Task.Run(() =>
            {
                try
                {
                    var authInfoForCall = new RemoteRecorderManagement.AuthenticationInfo
                        { UserKey = _soapAuthInfo.UserKey, Password = _soapAuthInfo.Password };
                    var recorderList =
                        _recorderManagementClient.GetRemoteRecordersById(authInfoForCall,
                            [remoteRecorderId]);
                    if (recorderList is { Length: > 0 } && recorderList[0] != null)
                        return recorderList[0].State.ToString();
                    return "Unknown";
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error GetRemoteRecorderState: {ex}");
                    return "Error";
                }
            });
        ...
        if (_soapAuthInfo != null)
            return await Task.Run(async () =>
            {
                try
                {
                    ...
                    var rrResponse =
                        await _recorderManagementClient.GetRemoteRecordersByIdAsync(authInfoForCall,
                            [remoteRecorderId]);
                    if (rrResponse.First().ScheduledRecordings.Length <= 0) return [];
    ...
    public async Task<List<Session>?> GetSessionDetailsAsync(Guid[] sessionIds)
    {
        ...
        if (_soapAuthInfo != null)
            return await Task.Run(() =>
            {
                try
                {
                    var authInfoForCall = new SessionManagement.AuthenticationInfo
                        { UserKey = _soapAuthInfo.UserKey, Password = _soapAuthInfo.Password };
                    var sessions = _sessionsClient.GetSessionsById(authInfoForCall, sessionIds);
                    return sessions is { Length: > 0 } ? sessions.ToList() : null;
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error GetSessionDetails: {ex}");
                    return null;
                }
            });
        ...
        if (_soapAuthInfo != null)
            return await Task.Run(() =>
            {
                try
                {
                    var authInfoForCall = new RemoteRecorderManagement.AuthenticationInfo
                        { UserKey = _soapAuthInfo.UserKey, Password = _soapAuthInfo.Password };
                    _recorderManagementClient.UpdateRecordingTime(authInfoForCall, sessionId,
                        newStartTime.ToUniversalTime(), newEndTime.ToUniversalTime());
                    Console.WriteLine(
                        $"UpdateSessionTimeAsync: Called UpdateRecordingTime for Session {sessionId} to Start: {newStartTime.ToUniversalTime()}, End: {newEndTime.ToUniversalTime()}");
                    return true;
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error UpdateSessionTime (via UpdateRecordingTime): {ex}");
                    return false;
                }
            });
    ...
    public async Task<Guid?> CreateRecordingAsync(Guid remoteRecorderId, string sessionName, DateTime startTime, TimeSpan duration, Guid folderId)
    {
        ...
        if (_soapAuthInfo != null)
            return await Task.Run(() =>
            {
                try
                {
                    var rrAuth = new RemoteRecorderManagement.AuthenticationInfo
                        { UserKey = _soapAuthInfo.UserKey, Password = _soapAuthInfo.Password };
    ...
    public async Task<Guid?> PauseSessionAsync(Guid internalSessionId)
    {
        ...
        try
        {
            ...
            response.EnsureSuccessStatusCode();
            var responseBody = await response.Content.ReadAsStringAsync();
            if (Guid.TryParse(responseBody.Trim('"'), out var pauseId)) return pauseId;
    ...
    private void Dispose(bool disposing)
    {
        ...
        if (disposing)
        {
            ...
            void CloseClient(object clientObj)
            {
                if (clientObj is not ICommunicationObject commObj) return;
                try
                {
                    if (commObj.State != CommunicationState.Faulted && commObj.State != CommunicationState.Closed)
                        commObj.Close();
